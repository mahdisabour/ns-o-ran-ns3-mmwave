...
main (int argc, char *argv[])
{
  ...
  uint32_t numUeNodes = ueNodes.GetN ();
  uint32_t groupSize = numUeNodes / 4;

  // Define the traffic parameters
  std::string fullBufferDataRate = "20Mbps"; // 20 Mbit/s
  std::string burstyHighDataRate = "3Mbps"; // 3 Mbit/s
  std::string burstyMediumDataRate = "750Kbps"; // 750 Kbit/s
  std::string burstyLowDataRate = "150Kbps"; // 150 Kbit/s
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      sinkApp.Add (dlPacketSinkHelper.Install (ueNode));
      ...
      if (u < groupSize)
        {
          // Full Buffer Traffic
          onOffHelper.SetAttribute ("DataRate", StringValue (fullBufferDataRate)); 
      ...
      else if (u < 2 * groupSize)
        {
          // Bursty Traffic (High Data Rate)
          onOffHelper.SetAttribute ("DataRate", StringValue (burstyHighDataRate)); 
          onOffHelper.SetAttribute (
              "OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1]")); 
          onOffHelper.SetAttribute (
              "OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1]")); 
        }
      else if (u < 3 * groupSize)
        {
          // Bursty Traffic (Medium Data Rate)
          onOffHelper.SetAttribute ("DataRate", StringValue (burstyMediumDataRate)); 
          onOffHelper.SetAttribute (
              "OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1]")); 
          onOffHelper.SetAttribute (
              "OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1]")); 
        ...
        {
          // Bursty Traffic (Low Data Rate)
          onOffHelper.SetAttribute ("DataRate", StringValue (burstyLowDataRate)); 
          onOffHelper.SetAttribute (
              "OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1]")); 
          onOffHelper.SetAttribute (
              "OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1]")); 
      ...
      ApplicationContainer clientApp = onOffHelper.Install (remoteHost);
